...
WEB::ApplePanelData WEB::appleBacklightData[] {
	{
		"F14Txxxx",
		{
...
void WEB::init() {
	...
	if (getKernelVersion() >= KernelVersion::Yosemite) {
		...
		if (resetFramebuffer >= FB_TOTAL) {
			SYSLOG("web", "invalid igfxrset value %d, falling back to autodetect", resetFramebuffer);
	...
	lilu.onPatcherLoadForce([](void *user, KernelPatcher &patcher) {
		static_cast<WEB *>(user)->processKernel(patcher);
	}, this);

	lilu.onKextLoadForce(nullptr, 0,
	[](void *user, KernelPatcher &patcher, size_t index, mach_vm_address_t address, size_t size) {
		static_cast<WEB *>(user)->processKext(patcher, index, address, size);
	}, this);
...
void WEB::processKernel(KernelPatcher &patcher) {
	...
	if (devInfo) {
		...
		if (graphicsDisplayPolicyMod == AGDP_DETECT) { /* Default detect only */
			auto getAgpdMod = [this](IORegistryEntry *device) {
				...
				if (prop) {
					DBGLOG("web", "found agdpmod in external GPU %s", safeString(device->getName()));
					...
					if (propStr) {
						...
						if (agdp && agdp[propData->getLength() - 1] != '\0') {
							DBGLOG("web", "agdpmod config is not null terminated");
							agdp = nullptr;
						}
					}
					if (agdp) {
		...
		if (devInfo->firmwareVendor != DeviceInfo::FirmwareVendor::Apple) {
			DBGLOG("web", "non-apple-fw proceeding with devprops %d", graphicsDisplayPolicyMod);
			...
			DBGLOG("web", "resulting applbkl value is %d", appleBacklightPatch);
			...
			// Support legacy -webtree argument.
			bool rebuidTree = checkKernelArgument("-webtree");
			...
			// Override with modern webtree argument.
			int tree;
			if (PE_parse_boot_argn("webtree", &tree, sizeof(tree)))
				rebuidTree = tree != 0;
			
			if (rebuidTree) {
				DBGLOG("web", "apple-fw proceeding with devprops by request");
	...
	if (resetFramebuffer == FB_COPY || resetFramebuffer == FB_ZEROFILL) {
		...
		if (info) {
			...
			SYSLOG("web", "failed to obtain vcinfo");
			patcher.clearError();
		}
	}
}

size_t WEB::wrapFunctionReturnZero() {
	return 0;
}

void WEB::processKext(KernelPatcher &patcher, size_t index, mach_vm_address_t address, size_t size) {
	if (kextIOGraphics.loadIndex == index) {
		...
		if (gIOFBVerboseBootPtr) {
			...
			SYSLOG("web", "failed to resolve gIOFBVerboseBoot");
			patcher.clearError();
		}
		return;
	}
	...
	if (kextBacklight.loadIndex == index) {
		...
		if (patcher.routeMultiple(kextBacklight.loadIndex, &request, 1, address, size)) {
			...
			DBGLOG("web", "applying backlight patch");
...
void WEB::processBuiltinProperties(IORegistryEntry *device, DeviceInfo *info) {
	...
	if (obj) {
		...
		if (realDevice != acpiDevice) {
			DBGLOG("web", "user requested to fake with normal device-id");
		...
		if (fakeDevice && obj->getProperty("no-gfx-spoof") == nullptr) {
			...
			if (fakeDevice != acpiDevice) {
				...
				SYSLOG("web", "IGPU device (%02X:%02X.%02X) has device-id 0x%04X, you should change it to 0x%04X",
		...
		SYSLOG("web", "invalid IGPU device type");
	}
	...
	if (BaseDeviceInfo::get().cpuGeneration == CPUInfo::CpuGeneration::Westmere) {
		if (!device->getProperty("AAPL,os-info")) {
			DBGLOG("web", "fixing AAPL,os-info");
			uint8_t osInfoBytes[] {
			...
			DBGLOG("web", "found existing AAPL,os-info");
		}
	}

	// Ensure built-in.
	if (!device->getProperty("built-in")) {
		DBGLOG("web", "fixing built-in");
		uint8_t builtBytes[] { 0x00 };
		device->setProperty("built-in", builtBytes, sizeof(builtBytes));
	} else {
		DBGLOG("web", "found existing built-in");
	}
}

void WEB::processExternalProperties(IORegistryEntry *device, DeviceInfo *info, uint32_t vendor) {
	auto name = device->getName();
...
void WEB::processManagementEngineProperties(IORegistryEntry *imei) {
	...
	if ((cpuGeneration == CPUInfo::CpuGeneration::SandyBridge ||
		...
		if (suggest != 0) {
			...
			SYSLOG("web", "IMEI device (%02X:%02X.%02X) has device-id 0x%04X, you should change it to 0x%04X",
				   bus, dev, fun, device, suggest);
		}
	}
}

void WEB::processGraphicsPolicyStr(const char *agdp) {
	DBGLOG("web", "agdpmod using config %s", agdp);
...
void WEB::processGraphicsPolicyMods(KernelPatcher &patcher, mach_vm_address_t address, size_t size) {
	if (graphicsDisplayPolicyMod & AGDP_VIT9696) {
		...
		if (patcher.getError() != KernelPatcher::Error::NoError) {
			SYSLOG("web", "failed to apply agdp vit9696's patch %d", patcher.getError());
			patcher.clearError();
		}
	}
	...
	if (graphicsDisplayPolicyMod & AGDP_PIKERA) {
		...
		patcher.applyLookupPatch(&patch);
		if (patcher.getError() != KernelPatcher::Error::NoError) {
			SYSLOG("web", "failed to apply agdp Piker-Alpha's patch %d", patcher.getError());
			patcher.clearError();
		}
	}
	...
	if (graphicsDisplayPolicyMod & AGDP_CFGMAP) {
		...
		if (getKernelVersion() >= KernelVersion::Mojave)
			PANIC("web", "adgpmod=cfgmap has no effect on 10.13.4, use agdpmod=ignore");
...
bool WEB::isGraphicsPolicyModRequired(DeviceInfo *info) {
	DBGLOG("web", "detecting policy");
	// Graphics policy patches are only applicable to discrete GPUs.
	if (info->videoExternal.size() == 0) {
		DBGLOG("web", "no external gpus");
		return false;
	}
	...
	for (size_t i = 0; i < arrsize(compatibleBoards); i++) {
		if (!strcmp(compatibleBoards[i], boardId)) {
			DBGLOG("web", "disabling nvidia patches on model %s", boardId);
			return false;
		}
	}
...
void WEB::wrapFramebufferInit(IOFramebuffer *fb) {
	bool backCopy = callbackWEB->gotConsoleVinfo && callbackWEB->resetFramebuffer == FB_COPY;
	bool zeroFill  = callbackWEB->gotConsoleVinfo && callbackWEB->resetFramebuffer == FB_ZEROFILL;
	auto &info = callbackWEB->consoleVinfo;
	...
	if (backCopy && info.v_baseaddr) {
		...
		callbackWEB->consoleBuffer = Buffer::create<uint8_t>(info.v_rowbytes * info.v_height);
		if (callbackWEB->consoleBuffer)
			lilu_os_memcpy(callbackWEB->consoleBuffer, reinterpret_cast<uint8_t *>(info.v_baseaddr), info.v_rowbytes * info.v_height);
		else
			SYSLOG("web", "console buffer allocation failure");
		// Even if we may succeed next time, it will be unreasonably dangerous
		info.v_baseaddr = 0;
	}

	uint8_t verboseBoot = *callbackWEB->gIOFBVerboseBootPtr;
	// For back copy we need a console buffer and no verbose
	backCopy = backCopy && callbackWEB->consoleBuffer && !verboseBoot;
	...
	if (backCopy || zeroFill) {
		...
		if (fb->getCurrentDisplayMode(&mode, &depth) == kIOReturnSuccess &&
			...
			if (info.v_rowbytes != pixelInfo.bytesPerRow || info.v_width != pixelInfo.activeWidth ||
				...
				DBGLOG("web", "this display has different mode");
			}
		} else {
			DBGLOG("web", "failed to obtain display mode");
			backCopy = zeroFill = false;
		}
	}

	// For whatever reason not resetting Intel framebuffer (back copy mode) twice works better.
	if (!backCopy) *callbackWEB->gIOFBVerboseBootPtr = 1;
	FunctionCast(wrapFramebufferInit, callbackWEB->orgFramebufferInit)(fb);
	if (!backCopy) *callbackWEB->gIOFBVerboseBootPtr = verboseBoot;

	// Finish the framebuffer initialisation by filling with black or copying the image back.
	if (FramebufferViewer::getVramMap(fb)) {
		auto src = reinterpret_cast<uint8_t *>(callbackWEB->consoleBuffer);
		auto dst = reinterpret_cast<uint8_t *>(FramebufferViewer::getVramMap(fb)->getVirtualAddress());
		if (backCopy) {
			DBGLOG("web", "attempting to copy...");
			...
			DBGLOG("web", "doing zero-fill...");
			memset(dst, 0, info.v_rowbytes * info.v_height);
		}
	}
}

uint16_t WEB::wrapConfigRead16(IORegistryEntry *service, uint32_t space, uint8_t offset) {
	auto result = callbackWEB->orgConfigRead16(service, space, offset);
	if (offset == WIOKit::kIOPCIConfigDeviceID && service != nullptr) {
		auto name = service->getName();
		if (!name) return result;
		bool doSpoof = (callbackWEB->hasIgpuSpoof && name[0] == 'I' && name[1] == 'G' && name[2] == 'P' && name[3] == 'U')
			|| (callbackWEB->hasGfxSpoof && name[0] == 'G' && name[1] == 'F' && name[2] == 'X');
		if (doSpoof) {
			DBGLOG("web", "configRead16 %s 0x%08X at off 0x%02X, result = 0x%04x", name, space, offset, result);
			uint32_t device;
			if (WIOKit::getOSDataValue(service, "device-id", device) && device != result) {
				DBGLOG("web", "configRead16 %s reported 0x%04x instead of 0x%04x", name, device, result);
				return device;
			}
		}
	}
...
uint32_t WEB::wrapConfigRead32(IORegistryEntry *service, uint32_t space, uint8_t offset) {
	auto result = callbackWEB->orgConfigRead32(service, space, offset);
	...
	if ((offset == WIOKit::kIOPCIConfigDeviceID || offset == WIOKit::kIOPCIConfigVendorID) && service != nullptr) {
		...
		bool doSpoof = (callbackWEB->hasIgpuSpoof && name[0] == 'I' && name[1] == 'G' && name[2] == 'P' && name[3] == 'U')
			|| (callbackWEB->hasGfxSpoof && name[0] == 'G' && name[1] == 'F' && name[2] == 'X');
		if (doSpoof) {
			DBGLOG("web", "configRead32 %s 0x%08X at off 0x%02X, result = 0x%08X", name, space, offset, result);
			uint32_t device;
			if (WIOKit::getOSDataValue(service, "device-id", device) && device != (result & 0xFFFF)) {
				device = (result & 0xFFFF) | (device << 16);
				DBGLOG("web", "configRead32 %s reported 0x%08x instead of 0x%08x", name, device, result);
				return device;
			}
		}
	}
...
bool WEB::wrapGraphicsPolicyStart(IOService *that, IOService *provider) {
	auto boardIdentifier = BaseDeviceInfo::get().boardIdentifier;

	DBGLOG("web", "agdp fix got board-id %s", boardIdentifier);
	...
	if (oldConfigMap) {
		...
		if (rawConfigMap) {
			...
			if (newConfigMap) {
				...
				SYSLOG("web", "agdp fix failed to clone ConfigMap");
			}
			rawConfigMap->release();
		}
	} else {
		SYSLOG("web", "agdp fix failed to obtain valid ConfigMap");
	}

	bool result = FunctionCast(wrapGraphicsPolicyStart, callbackWEB->orgGraphicsPolicyStart)(that, provider);
	DBGLOG("web", "agdp start returned %d", result);

	return result;
}

bool WEB::wrapApplePanelSetDisplay(IOService *that, IODisplay *display) {
	if (!callbackWEB->applePanelDisplaySet) {
		callbackWEB->applePanelDisplaySet = true;
		...
		if (panels) {
			...
			if (panels) {
				...
				if (callbackWEB->appleBacklightCustomName != nullptr) {
					auto length = callbackWEB->appleBacklightCustomName->getLength();
					const char *customNameBytes = static_cast<const char *>(callbackWEB->appleBacklightCustomName->getBytesNoCopy());
				...
				for (auto &entry : appleBacklightData) {
					if (customName != nullptr && strcmp(customName, entry.deviceName) == 0) {
						panels->setObject(entry.deviceName, callbackWEB->appleBacklightCustomData);
						DBGLOG("web", "using custom panel data for %s device", entry.deviceName);
					} else {
						...
						if (pd) {
							...
							SYSLOG("web", "panel start cannot allocate %s data", entry.deviceName);
						}
					}
				}
			...
			SYSLOG("web", "panel start has no panels");
		}
	}

	bool result = FunctionCast(wrapApplePanelSetDisplay, callbackWEB->orgApplePanelSetDisplay)(that, display);
	DBGLOG("web", "panel display set returned %d", result);

	return result;
}

bool WEB::getVideoArgument(DeviceInfo *info, const char *name, void *bootarg, int size) {
